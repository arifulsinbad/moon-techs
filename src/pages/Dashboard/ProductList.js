...
import { getProduct, removeProduct, toggleDeleteSuccess, toggleUpdate } from "../../features/products/productsSlice";
import { toast } from "react-hot-toast";
import { Link } from "react-router-dom";

const ProductList = () => {
 const dispatch = useDispatch()
 const {products, deleteSuccess, isLoading, isError, error } = useSelector((state)=> state.products)
  ...
  useEffect(() => {
    if(!isLoading && deleteSuccess){
      toast.success('Delete Success', {id : 'ProductList'})
      dispatch(toggleDeleteSuccess())
    }
    if(!isLoading && isError){
      toast.error(error, {id : 'ProductList'})
    }
  },[ isLoading, deleteSuccess, dispatch, isError, error]);
  if(isLoading){
    return <p>Loading...</p>
  }
            ...
            <tbody class='text-sm divide-y divide-gray-100'>
              {products.map(({ model, brand, price, status, _id }) => (
                <tr>
                  ...
                  <td><Link to='/dashboard/update-product'><button className="btn border-t-green-500" onClick={()=>dispatch(toggleUpdate(_id))}>Edit</button></Link></td>
                  <td class='p-2'>
