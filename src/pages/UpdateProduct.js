...
const {isLoading, isError, error, updateSuccess, updateId}=useSelector((state)=>state.products)
  const submit = (data) => {
...
dispatch(productUpdate(updateId, product))
    console.log(product);
  };

  useEffect(()=>{
    if(isLoading){
      toast.loading('Loading...', {id : 'UpdateSuccess'})
    };
    if(!isLoading && updateSuccess){
      toast.success('UpdateSuccess', {id: 'UpdateSuccess'})
      dispatch(toggleUpdate())
    };
    if(!isLoading && isError){
      toast.error(error, {id :'UpdateSuccess'})
    }
  },[isLoading, updateSuccess,dispatch, isError, error])
